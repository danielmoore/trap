...
function onRun() {
  if (this.node.skip) {
    ...
    this.out({
      format: [ '%s▹ %s', this.indent, colors.underline(this.node.description) ],
      colors: [ 'yellow' ]
    });
  } else {
    this.state.testsRun++;

    this.out('%s▸ %s', this.indent, colors.underline(this.node.description));
...
function onAssertOk(text) {
  ...
  this.out({
    format: [ '%s✔ %s', this.childIndent, text || 'ok' ],
    colors: [ 'green' ]
  });
}
...
function onAssertNotOk(text) {
  ...
  this.out({
    format: [ '%s✘ %s', this.childIndent, text || 'not ok' ],
    colors: [ 'red', 'bold' ]
  });
}
...
function onComment(args) {
  this.out({
    format: ['%s✳ %s', this.childIndent, sprintf.apply(null, args)],
    colors: [ 'brightBlack' ]
  })
}
...
function Writer(config, state, node, indentCount) {
  ...
  this.childIndent = getIndent(indentCount + 1);
}
...
Writer.prototype.out = function (format) {
  if (arguments.length === 1 && typeof format === 'object') {
    ...
    var applyColors = arguments[0].colors;
  ...
  if (applyColors && !this.config.noColors)
    str = applyColors.reduce(function (str, c) { return colors[c](str); }, str);
...
module.exports = function (runner, config) {
  ...
  runner.on('done', function () {
    ...
    if (!state.testsFailed && !state.assertionsFailed) {
      ...
      out('  %s %s', colors.gree(passStr), skippedStr);
    } else {
      out(colors.red(colors.bold(sprintf(
        '  ✘ Fail: %s, %s',
